...
public class TUT_ChadMovement : MonoBehaviour
{
    ...
    public Vector2 JumpStart { get; private set; }
    ...
    {
        Debug.Log("Calling Kick  1!!!");
        if (kickCronometer <= 0)
        {
            Debug.Log("Calling Kick  2!!!");
            if (inAir != true)
            {
                Debug.Log("Calling Kick  3!!!");
                if (gameManager.GetFirstAnimal() != null)
                {
                    Debug.Log("Calling Kick  4!!!");
                    if (CheckIfAnimalNear(gameManager.GetFirstAnimal()))
                    {
                        FindObjectOfType<AudioManager>().Play("chadPatada");

                        Debug.Log("Calling Kick   5!!!");
                        TUT_AnimalState animScript = gameManager.GetFirstAnimal().GetComponent<TUT_AnimalStateMachine>().CurrentState;
